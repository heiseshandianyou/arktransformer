export class StringMap{
    ...
    // A/B/C.ts => A.B.Cts
    public static PackageMap(input: string): string{
    ...
    public static FileNameMap(input:string):string{
        return input.replace(/\s+/g, '')
                    .replace(/\./g, '')
                    .replace(/\//g, '.');
    }
...
export class Map{
    ...
    public static NameSpaceSigMap(input: string): string{
        return input.replace(/-/g, '_');
    }
    ...
    public static modifierMap(input: string):string{
    ...
    //这个方法用来消除方法名中的@
    public static methodNmaeMap(input: string):string{
    ...
    //这个方法用来消除参数名名中的@
    public static localNameMap(input: string):string{
        return (input.replace(/[@]/g, ''));
    }
